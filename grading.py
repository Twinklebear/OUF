...
def regrade(problem, stdout_file, result_file, grade_file, ref_stdout_file, editor):
    if os.path.isfile(grade_file):
        ...
        grade(problem, stdout_file, result_file, grade_file, ref_stdout_file, editor)
...
def compile(cl_stdout_file, problem, name):
    with open(cl_stdout_file, 'w') as cl_stdout:
        try:
            subprocess.Popen(['cl.exe', '/W4', '/EHsc', '/Zi', '/GL', '/Fe' + name] + problem["files"],
...
def run_student(exe, stdin_file, stdout_file, cl_stdout_file):
    ...
    if statinfo.st_size > 8 * 1024:
        with open(stdout_file, 'w') as f:
            ...
            f.write("YOUR PROCESS PRODUCED BEYOND 8KB OF OUTPUT AND OUTPUT WAS IGNORED\n")
